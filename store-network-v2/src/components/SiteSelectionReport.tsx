...
function computeSiteSelectionScore(
  ...
  const avgCostList: number[] = [];
  for (const s of stores) {
    if (s.market) {
      if (typeof s.market.residential_count === 'number') residentialList.push(s.market.residential_count);
      if (typeof s.market.avg_cost === 'number' && s.market.avg_cost > 0) avgCostList.push(s.market.avg_cost);
    }
  }
  ...
  // ---- 1. 商圈潜力（0-35 分）— residential_count ----
  ...
  if (nearestStore?.market && typeof nearestStore.market.residential_count === 'number') {
    ...
    potentialScore = scoreByPercentile(residentialPct, 35);
  }

  // ---- 2. 商圈消费力（0-30 分）— avg_cost ----
  let spendingScore = 0;
  let avgCostPct = 0;
  ...
  if (nearestStore?.market && typeof nearestStore.market.avg_cost === 'number' && nearestStore.market.avg_cost > 0) {
    ...
    spendingScore = scoreByPercentile(avgCostPct, 30);
  }
  ...
  if (totalCompetitors === 0) compScore = 2;
  else if (totalCompetitors <= 5) compScore = 4;
  else if (totalCompetitors <= 15) compScore = 6;
...
export default function SiteSelectionReport({ lat, lng }: SiteSelectionReportProps) {
  ...
  // 关键结论：找出最拖分和最加分的维度
  const dimensionGaps = [
    { name: '商圈潜力', score: analysis.potentialScore, max: 35 },
      ...
      <div style={{ marginBottom: 12 }}>
        ...
        <div style={{ marginBottom: 12, padding: 8, background: '#f8fafc', borderRadius: 6 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, marginBottom: 4 }}>
            ...
            <span style={{ fontWeight: 600 }}>{analysis.potentialScore}/35</span>
          </div>
          <div style={{ height: 4, background: '#e2e8f0', borderRadius: 2, overflow: 'hidden', marginBottom: 6 }}>
            <div style={{ height: '100%', width: `${(analysis.potentialScore / 35) * 100}%`, background: '#3b82f6' }} />
          </div>
          <div style={{ fontSize: 10, color: '#64748b', marginBottom: 4 }}>
            📊 数据：1km 内住宅小区 {analysis.residentialValue ?? '—'} 个
            （全部门店前 {analysis.residentialPct}%，中位数 {analysis.medianResidential}）
            {analysis.businessArea && <span> · 商圈：{analysis.businessArea}</span>}
          </div>
          <div style={{ fontSize: 10, color: '#94a3b8', lineHeight: 1.5 }}>
            💡 评分规则：住宅小区 POI 数按百分位打分（前 10% 满分、前 25% 较高、前 50% 中等、前 75% 较低、后 25% 低）
          </div>
        ...
        <div style={{ marginBottom: 12, padding: 8, background: '#f8fafc', borderRadius: 6 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, marginBottom: 4 }}>
            ...
            <span style={{ fontWeight: 600 }}>{analysis.spendingScore}/30</span>
          </div>
          <div style={{ height: 4, background: '#e2e8f0', borderRadius: 2, overflow: 'hidden', marginBottom: 6 }}>
            <div style={{ height: '100%', width: `${(analysis.spendingScore / 30) * 100}%`, background: '#8b5cf6' }} />
          </div>
          <div style={{ fontSize: 10, color: '#64748b', marginBottom: 4 }}>
            📊 数据：商圈人均消费 ¥{analysis.avgCostValue ?? '—'}
            （全部门店前 {analysis.avgCostPct}%，中位数 ¥{Math.round(analysis.medianAvgCost)}）
          </div>
          <div style={{ fontSize: 10, color: '#94a3b8', lineHeight: 1.5 }}>
            💡 评分规则：商圈内餐厅人均消费均价按百分位打分。Wagas 客单价 60-80 元，需消费力匹配的商圈。
          </div>
        </div>
        ...
        <div style={{ marginBottom: 12, padding: 8, background: '#f8fafc', borderRadius: 6 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, marginBottom: 4 }}>
            ...
            <span style={{ fontWeight: 600 }}>{analysis.compScore}/10</span>
          </div>
          <div style={{ height: 4, background: '#e2e8f0', borderRadius: 2, overflow: 'hidden', marginBottom: 6 }}>
            <div style={{ height: '100%', width: `${(analysis.compScore / 10) * 100}%`, background: '#f59e0b' }} />
          </div>
          <div style={{ fontSize: 10, color: '#64748b', marginBottom: 4 }}>
        ...
        <ul style={{ paddingLeft: 20, margin: 0 }}>
          ...
          <li style={{ marginBottom: 4 }}>
            ...
            {weakest.name === '商圈潜力' && analysis.residentialValue !== null && (
              <span>，住宅小区 {analysis.residentialValue} 个（前 {analysis.residentialPct}%）</span>
            )}
        ...
        <ul style={{ paddingLeft: 20, margin: 0 }}>
          ...
          {analysis.residentialValue !== null && (
            <li style={{ marginBottom: 6 }}>
              <strong>商圈潜力：</strong>
              周边 1km 内 {analysis.residentialValue} 个住宅小区（全部门店前 {analysis.residentialPct}%）。
              {analysis.residentialValue > analysis.medianResidential
                ? '住宅密度高于中位数，有稳定的常住人口基础。'
                : '住宅密度低于中位数，日常客流可能不足，建议关注写字楼和商业流量补充。'}
            </li>
          )}
          ...
          {analysis.avgCostValue !== null && (
            <li style={{ marginBottom: 6 }}>
              <strong>商圈消费力：</strong>
              商圈人均消费 ¥{analysis.avgCostValue}（前 {analysis.avgCostPct}%）。
