...
// ====== 多选下拉组件 ======
var msData = {{
  name: {{ items: [], selected: [], page: 1, perPage: 15, filtered: [], fullItems: stores.map(function(s){{return {{id:s.sid, label:s.name, brand:s.brand, city:s.city}}}}) }},
  sid: {{ items: [], selected: [], page: 1, perPage: 15, filtered: [], fullItems: stores.map(function(s){{return {{id:s.sid, label:s.sid, brand:s.brand, city:s.city}}}}) }}
}};
// 初始化：从 fullItems 复制到 items
msData.name.items = msData.name.fullItems.slice();
msData.sid.items = msData.sid.fullItems.slice();
...
// ====== 筛选器联动：品牌/城市改变时更新门店多选列表 ======
function updateMsOptions(){{
  var bf=document.getElementById("brand-filter").value;
  var cf=document.getElementById("city-filter").value;
  ["name","sid"].forEach(function(type){{
    var prevSel=msData[type].selected.slice();
    msData[type].items=msData[type].fullItems.filter(function(item){{
      if(bf!=="all"&&item.brand!==bf&&item.brand!==bf.replace("&"," & "))return false;
      if(cf!=="all"&&item.city!==cf)return false;
      return true;
    }});
    var newIds=msData[type].items.map(function(i){{return i.id}});
    msData[type].selected=prevSel.filter(function(id){{return newIds.indexOf(id)>=0}});
    msData[type].filtered=msData[type].items.slice();
    msData[type].page=1;
    msRender(type);
  }});
...
function loadTopLoc(sid,btn){{
  function renderTable(data){{
    ...
    h+='<div style="font-weight:700;color:#1e40af;margin-bottom:3px">热门配送地 TOP10 <span style="font-weight:400;color:#94a3b8;font-size:8px">(全量单数)</span></div>';
